...
class TestPaserUrl {
    ...
    public void checkForCorrectOutputTitle() throws IOException {
        ...
        SourceFormat sourceFormat = new SourceFormat(InputSource.URL, InputFormat.NOTSIMPLE);
        ParserUrl parser = (ParserUrl) sourceFormat.accept(json);
        String title = parser.getArticles().get(0).getTitle();

        assertTrue("The latest on the coronavirus pandemic and vaccines: Live updates - CNBC".equals(title));
    }
    ...
    public void checkForCorrectOutputUrl() throws IOException {
        ...
        assertTrue("https://www.cnbc.com/world/live-news/coronavirus-pandemic-vaccine-updates-03-24-21/index.html"
                .equals(url));
    }
     ...
     */
    @Test
    public void checkForCorrectOutputTitlerWithFilter() throws IOException {
        GetJsonFromUrl getJson = Mockito.mock(GetJsonFromUrl.class);
        when(getJson.getJsonStrFromUrl(dummyUrl)).thenReturn(jsonStr);

        String json = getJson.getJsonStrFromUrl(dummyUrl);

        SourceFormat sourceFormat = new SourceFormat(InputSource.URL, InputFormat.NOTSIMPLE, InputArticleSource.CNBC);
        ParserUrl parser = (ParserUrl) sourceFormat.accept(json);
        String title = parser.getArticles().get(0).getTitle();
        System.out.print(parser.getArticles());
